...
- 在网页翻译页新增“不自动翻译网站 · 设备同步”，使用加密 GitHub Secret Gist 每日同步域名列表。
- 同步范围严格限制为 `transOpen: "false"` 的非全局网站规则；语言、翻译引擎、API、快捷键、调优、字幕及其他规则字段全部保留在本机。
- 按域名合并多设备变更并保留删除墓碑，避免离线旧设备恢复已删除的网站。
